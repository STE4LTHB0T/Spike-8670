...
class Music(commands.Cog):
    ...
    async def _queue(self, ctx):
        ...
        total_runtime = utils.format_time(sum([int(song.info.duration if song.info.duration else 0) for song in list(playlist.playque)]))
    ...
    @commands.command(name='clearing', description=config.HELP_CLEAR_LONG, help=config.HELP_CLEAR_SHORT, aliases=['cl'])
